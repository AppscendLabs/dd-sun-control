  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
        <svg
          key={star}
          className={`${px} ${star <= rating ? "text-yellow-400" : "text-slate-300"}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12 text-center">
          ...
          >
            ...
            {data?.total ? ` · ${data.total} total reviews` : ""}
          </motion.p>
        </div>
        ...
        {loading && (
          <div className="mb-10 grid grid-cols-1 gap-6 md:grid-cols-3">
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className="animate-pulse rounded-2xl bg-slate-800 p-6"
              >
                <div className="mb-4 flex gap-1">
                  {[...Array(5)].map((_, j) => (
                    <div key={j} className="h-4 w-4 rounded bg-slate-700" />
                  ))}
                </div>
          ...
          <div className="mb-10 grid grid-cols-1 gap-6 md:grid-cols-3">
            {reviews.map((review, index) => (
              <motion.div
                ...
                className="flex flex-col rounded-2xl bg-slate-800 p-6"
              >
                <div className="mb-3">
        ...
        <div className="text-center">
          <a
            ...
            className="inline-flex items-center gap-2 text-sm text-slate-400 transition-colors hover:text-yellow-400"
          >
